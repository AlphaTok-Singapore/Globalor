import{d as _}from"./index-DrFu-skq.js";import{r as g}from"./index-GiUgBvb1.js";import{c as k}from"./client-B5L6vJYF.js";import"./index-CROobee-.js";const{useParameter:C,addons:S,useEffect:P,useMemo:H,definePreview:G}=__STORYBOOK_MODULE_PREVIEW_API__,{deprecate:E}=__STORYBOOK_MODULE_CLIENT_LOGGER__;var R=Object.defineProperty,v=(r,t)=>{for(var e in t)R(r,e,{get:t[e],enumerable:!0})},T={};v(T,{initialGlobals:()=>N});var h="themes",A=`storybook/${h}`,w="theme",f={},I={REGISTER_THEMES:`${A}/REGISTER_THEMES`},N={[w]:""},$={};v($,{initializeThemeState:()=>x,pluckThemeFromContext:()=>y,useThemeParameters:()=>M});function y({globals:r}){return r[w]||""}function M(r){return E(_`The useThemeParameters function is deprecated. Please access parameters via the context directly instead e.g.
    - const { themeOverride } = context.parameters.themes ?? {};
    `),r?r.parameters[h]??f:C(h,f)}function x(r,t){S.getChannel().emit(I.REGISTER_THEMES,{defaultTheme:t,themes:r})}var L="html",b=r=>r.split(" ").filter(Boolean),j=({themes:r,defaultTheme:t,parentSelector:e=L})=>(x(Object.keys(r),t),(l,c)=>{let{themeOverride:i}=c.parameters[h]??{},s=y(c);return P(()=>{let d=i||s||t,a=document.querySelector(e);if(!a)return;Object.entries(r).filter(([p])=>p!==d).forEach(([p,n])=>{let o=b(n);o.length>0&&a.classList.remove(...o)});let u=b(r[d]);u.length>0&&a.classList.add(...u)},[i,s]),l()}),U={"panel-BpInY05d.js":`import { jsx, jsxs } from "react/jsx-runtime";
...
`};function O(r={}){if(!(typeof window<"u")){console.warn("Stagewise Toolbar is not supported in non-browser environments.");return}if(document.querySelector("stagewise-toolbar")){console.warn("Stagewise Toolbar is already loaded - aborting init.");return}const t=document.createElement("stagewise-toolbar");t.style.display="block",t.style.position="fixed",t.style.top="0",t.style.left="0",t.style.right="0",t.style.bottom="0",t.style.width="100vw",t.style.height="100vh",t.style.zIndex="2147483647",t.style.pointerEvents="none";const e=document.createElement("iframe");e.style.display="block",e.style.border="none",e.style.overflow="hidden",e.style.margin="0",e.style.padding="0",e.style.width="100vw",e.style.height="100vh",e.style.backgroundColor="transparent",e.style.pointerEvents="none",e.style.colorScheme="normal",e.sandbox.add("allow-same-origin"),e.sandbox.add("allow-scripts"),e.sandbox.add("allow-presentation"),e.sandbox.add("allow-pointer-lock"),e.setAttribute("allowtransparency","true"),e.srcdoc='<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link rel="preconnect" href="https://rsms.me/"><link rel="stylesheet" href="https://rsms.me/inter/inter.css"></head><body style="pointer-events: none;"></body></html>',e.addEventListener("load",()=>{let l=!1;const c=n=>{const o=e.contentDocument.elementFromPoint(n.clientX,n.clientY),m=o&&o!==document.body&&o.tagName!=="HTML";m!==l&&(e.style.pointerEvents=m?"auto":"none",l=m)};window.addEventListener("mousemove",c,{capture:!0}),e.contentWindow.addEventListener("mousemove",c,{capture:!0});const i="",s=Object.fromEntries(Object.entries(U).map(([n,o])=>[n,URL.createObjectURL(new Blob([o],{type:"text/javascript"}))]));if(s["@stagewise/toolbar/plugin-ui"]=URL.createObjectURL(new Blob(["export * from 'plugin-ui.js'"],{type:"text/javascript"})),r.plugins)for(const[n,o]of r.plugins.entries()){const m=URL.createObjectURL(new Blob([o.mainPlugin],{type:"text/javascript"}));s[`plugin-entry-${n}`]=m}const d=URL.createObjectURL(new Blob([Z(r)],{type:"text/javascript"}));s["@stagewise/toolbar/config"]=d;const a={react:`https://esm.sh/react@19.1.0${i}`,"react-dom":`https://esm.sh/react-dom@19.1.0${i}`,"react-dom/client":`https://esm.sh/react-dom@19.1.0/client${i}`,"react/jsx-runtime":`https://esm.sh/react@19.1.0/jsx-runtime${i}`,...s},u=e.contentDocument.createElement("script");u.type="importmap",u.textContent=`{"imports":${JSON.stringify(a)}}`,e.contentDocument.head.appendChild(u);const p=e.contentDocument.createElement("script");p.type="module",p.textContent="import('index.js');",e.contentDocument.head.appendChild(p)}),t.appendChild(e),document.body.appendChild(t)}function Z(r){var t,e;const l=((t=r.plugins)==null?void 0:t.map((d,a)=>`import plugin${a} from 'plugin-entry-${a}'`).join(`
`))??"",c=`[${((e=r.plugins)==null?void 0:e.map((d,a)=>`plugin${a}`).join(","))??""}]`,i={...r,plugins:"__PLUGIN_PLACEHOLDER__"};let s=JSON.stringify(i);return s=s.replace('"__PLUGIN_PLACEHOLDER__"',c),`${l}

const config = ${s};

export default config;
`}function V({config:r,enabled:t=!1}){const e=g.useRef(!1);return g.useEffect(()=>{e.current||!t||(e.current=!0,O(r))},[r,t]),null}const D=r=>{if(typeof window<"u"&&!document.getElementById("stagewise-toolbar-root")){const t=document.createElement("div");t.id="stagewise-toolbar-root",document.body.appendChild(t);const e={plugins:[]};k(t).render(g.createElement(V,{config:e}))}return r()},K={parameters:{controls:{matchers:{color:/(background|color)$/i,date:/Date$/i}},darkMode:{classTarget:"html",darkClass:"dark",lightClass:"light"}},decorators:[D,j({themes:{light:"light",dark:"dark"},defaultTheme:"light"})]};export{K as default};
